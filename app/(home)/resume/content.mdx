...
- **小程序:**
  ...
  - 使用 `Taro.connectSocket` 完成用户即时通讯功能, 并且使用 `Zustand` 和小程序本地缓存高效管理即时通讯的数据状态。
  - 基于 `YunXinMiniappSDK.Client` 实现音视频通话。
  - 封装 `setup-static` 脚本, 自动上传静态资源到服务器, 并且实现ide预览功能。
